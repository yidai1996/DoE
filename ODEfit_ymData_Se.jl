...
function loadProcessData()
    ...
    xf=XLSX.readxlsx("G:\\My Drive\\Research\\DOE project\\Modeling\\Data fitting\\Monoculture with union media\\Se\\20221121_UM_OD.xlsx")
    sh1=xf["OD"]
    # sh2=xf["Iso_Prod_1"]
    # sh3=xf["OD_2"]
    # sh4=xf["Iso_Prod_2"]
    global d1=24*convert(Array{Float64,2},sh1["M13:M19"])
    global Se1=0.185*convert(Array{Float64,2},sh1["Q13:Q19"]) # g/L
    global Se2=0.185*convert(Array{Float64,2},sh1["R13:R19"])
    # global N1=(14+4)*0.001*convert(Array{Float64,2},sh1["D27:D29"])
    # global N2=(14+4)*0.001*convert(Array{Float64,2},sh1["E27:E29"])

    global X0=[Se1[1] Se2[1]] # g/L
    global N0=[3 3 3 3] # g/L
...
function test1(tt,a)# Simple model: only sucrose limited
    ...
    # y=[A S N]
    f(y,p,t)=[(a[1]*y[2]/(a[2]+y[2]) - a[5])*y[1],# X(Se)
    # f(y,p,t)=[(a[1]*y[2]/(a[2]+y[2]))*y[1],# X(Se)
         max(y[2],0)/y[2]*(- (a[1]*y[2]/(a[2]+y[2])/a[4]+a[3])*y[1])] # Sucrose
    # prob=ODEProblem(f,[X0[1],S0[1],PO[1]],(0,d1[end]))
    prob1=ODEProblem(f,[X0[1],N0[1]],(0,d1[end]))
...
function ODEStep1(X,N,tspan,a) # Use one ODE solver to solve the whole system
    f(y,p,t)=[(a[1]*y[2]/(a[2]+y[2]) - a[5])*y[1], # X(Se)
                max(y[2],0)/y[2]*(-(a[1]*y[2]/(a[2]+y[2])/a[4] + a[3])*y[1])] # Sucrose
    prob1=ODEProblem(f,[X[1],N[1]],(0.0,tspan))
    prob2=ODEProblem(f,[X[2],N[2]],(0.0,tspan))
    ...
    c2=soln2.t
    A2=Array(soln2)
    p1=plot(c1,A1[1,:],title="S.e growth curve fitting",ylabel="S.e Concentration (g/L)",xlabel="Time (hrs)",label="Fitting line of exper a&b",legend=:bottomright,framestyle=:box)
    # p2=plot!(c2,A2[1,:],label="Fitting line of exper b")
    scatter!(d1,Se1,kind="scatter",label="Experimental data of a")
    scatter!(d1,Se2,kind="scatter",label="Experimental data of b")
    display(p1)
    savefig("S.e growth curve fitting.pdf")
    ...
    # display(ps)
    savefig("Nitrate comsumption curve fitting.pdf")
...
CC=vec(cat(Se1,Se1,dims=1))
tt=vec(cat(d1,d1,dims=1))
# CC=vec(cat(Se2,Se2,dims=1))
# lower_p=[0.0,0.0,0.0,0.0,0.525]
# p0=[.05,5,.2,.5,.56]
# p0=[0.3,0.5,0.1,5]
# p0=[0.108 0.787 0.52 2.27]
p0=[0.02, 0.4, 0.8, 2, 0.000] # should be a good starting point
Parameters=Datafit(CC,p0,tt)
ODEStep1(X0,N0,d1[end],Parameters)
# ODEStep1(X0,N0,d1[end],p0)