...
function loadProcessData(N9,N10,N11,N12)
    ...
    global tspan1=2000
    global tspan2=100
    global out_dir="G:\\My Drive\\Research\\DOE project\\Modeling\\DimensionlessAnalysis\\test0129deathrate\\"
...
# function BicultureGrowth(N0, C0, A0_given, S0_given; filename = "BatchBiculture") # Batch flow
    ...
    println(out_dir)
    Plots.savefig(out_dir * "Microbial_" * filename * "_S0_$(@sprintf("%.3f",C0))_N0_$(@sprintf("%.3f",N0))_tc_$(@sprintf("%.2f",c1)) XSec_$(@sprintf("%.2f",c2)) XAvc_$(@sprintf("%.2f",c3)) CSc_$(@sprintf("%.2f",c4)) CAs_$(@sprintf("%.2f",c5)) YsA_$(@sprintf("%.2f",Ys_Av)) YsS_$(@sprintf("%.2f",Ys_Se)) YpA_$(@sprintf("%.2f",Yp_Av)) YpS_$(@sprintf("%.2f",Yp_Se)) N9_$(@sprintf("%.2f",Dimensionless_Numbers[9])) N10_$(@sprintf("%.2f",Dimensionless_Numbers[10])) N11_$(@sprintf("%.2f",Dimensionless_Numbers[11])) N12_$(@sprintf("%.2f",Dimensionless_Numbers[12])).png")
    Plots.plot(tt1,Ct1,label="BP",xaxis="Dimensionless Time",yaxis="Dimensionless Nutrient Concentration",framestyle=:box#=, legend=:topleft=#)
    Plots.plot!(tt1,Nt1,label="AP")
...
# 01/09/25
BicultureGrowth(0.15, 0.07, 0.01, 0.01, 1, 1, 0, 0; filename = "kd_0.1_C_S_c_Ks_Av_0131")
