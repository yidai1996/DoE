...
function loadProcessData()
    ...
    global saioutE=0.0025 # David's thesis
    global saioutS=0.0025
    global saioutA=0.0025
    global V=0.1 #L 100 mL
    global Vt=2*V
    # global Vt=3*V
    ...
    global out_dir="C:\\Users\\yid\\TemporaryResearchDataStorage\\doe61_tspan_200\\"
...
function Startup(A,S,N,C,tspan1,pS,pA) # batch
    ...
    k = 100
    # f(y,p,t)=[ 1/(1 + exp( - k*y[1]))* (pA[1]*y[3]/(pA[2]+y[3]) - kdA)*y[1],# X(Av)
    #      1/(1 + exp( - k*y[2])) * (pS[1]*y[4]/(pS[2]+y[4]) - kdS)*y[2],# X(Se)
    ...
    #      (y[2]+abs(y[2]))/2 * ( - kdS)*y[2] + pS[1]*y[4]/(pS[2]+y[4])*y[2],# X(Se)
    #      (y[3]+abs(y[3]))/2 *( - (pA[1]*y[3]/(pA[2]+y[3])/pA[3]+pA[5])*y[1]) + pS[4]*(pS[1]*y[4]/(ksS+y[4]))/pS[3]*y[2], # Sucrose
    #      (y[4]+abs(y[4]))/2 *( - (pS[1]*y[4]/(pS[2]+y[4])/pS[3]+pS[5])*y[2]) + pA[4]*pA[1]*y[3]/(pA[2]+y[3])*y[1]/pA[3]] # Ammonia
